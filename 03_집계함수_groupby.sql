...
select count(*), -- emp 테이블 행의 개수
	   count(salary), -- salary 테이블 행의 수
       count(comm_pct) -- null을 뺀 원소개수
from emp;

-- comm_pct 평균
select avg(comm_pct),
	   sum(comm_pct) / count(comm_pct),
       sum(comm_pct) / count(*)
from emp;
-- ∴ avg()는 값있는 것만 계산한다.

-- EMP 테이블에서 가장 최근 입사일(hire_date)과 가장 오래된 입사일을 조회
select max(hire_date) "최초입사일",
	   min(hire_date) "최고입사일"
from emp;  
...
select count(*) - count(comm_pct) from emp; -- null 값 조회 함수는 없음
...
select count(distinct dept_name) from emp; -- null 빼고 count 
select count(distinct ifnull(dept_name, 0)) from emp; -- null 포함 

/*
null : 모르는 값 (수집이 안된 값)
null : 어떤 의미를 가지는 값으로 사용 => 0, 없다 (이 경우 count 시 개수에 포함이 되야함)
*/
...
select case when salary < 5000 then '5000 미만'
			  when salary between 5000 and 9999 then '5000이상 10000 미만'
              when salary between 10000 and 19999 then '10000이상 20000 미만'
			  else '20000 이상' end as "범위",
	   count(*)
from emp
...
-- 직원수가 10명 이상인 부서의 부서명과 그 부서 직원들의 급여 평균.
select  dept_name,
		avg(salary) "평균급여", 
        count(*) "직원수"
from	emp
...
group by dept_name
having count(*) >= 10 -- 이 집계 결과 조건을 만족하는 그룹만 Fitlering 한다.
order by 1;
...
select  if(grouping(mgr_id)=1, '총계', mgr_id),
		count(*) "직원수"
from    emp
