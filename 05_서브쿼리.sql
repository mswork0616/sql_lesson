...
1. sub query가 먼저 실행
2. sub query의 조회 결과릉 이용하여 main query 실행 
************************************************************************** */
...
select d.dept_name,
	   concat('$', format(avg(e.salary), 2)) -- concat(), format() : 문자열
from dept d join emp e on d.dept_id = e.dept_id
group by d.dept_id, d.dept_name
having avg(e.salary) >= (select avg(salary) from emp)
order by 2; -- 문자열이기 때문에 정렬 불가
...
from (
	...
	from dept d join emp e on d.dept_id = e.dept_id
	group by d.dept_id, d.dept_name
	having (select avg(salary) from emp)
	order by 2
) t;
...
-- TODO: 직원의 ID(emp.emp_id)가 150인 직원과 업무(emp.job_id)와 상사(emp.mgr_id)가 같은 직원들의 id(emp.emp_id), 이름(emp.emp_name), 업무(emp.job_id), 상사(emp.mgr_id) 를 조회
select emp_id,
	   emp_name,
       job_id,
       mgr_id
...
-- TODO : EMP 테이블에서 직원 이름이(emp.emp_name)이  'John'인 직원들 중에서 -- 급여(emp.salary)가 가장 높은 직원의 salary(emp.salary)보다 많이 받는 직원들의 id(emp.emp_id), 이름(emp.emp_name), 급여(emp.salary)를 조회.
select emp_id,
	   emp_name,
       salary
from emp
...
select *
from emp
where ifnull(job_id, 'none') != 'ST_CLERK' and salary < (select avg(salary) from emp where job_id = 'ST_CLERK');
...
-- TODO: 전체 직원들 중 'IT' 부서(dept.dept_name)의 직원중 가장 많은 급여를 받는 직원보다 더 많이 받는 직원의 ID(emp.emp_id), 이름(emp.emp_name), 입사일(emp.hire_date), 부서 ID(emp.dept_id), 급여(emp.salary) 조회
...
select emp_id from emp where emp_name = 'Alexander'; -- 행이 2개 
...
- 보통 데이터테이블의 값이 내역테이블(Transaction TB)에 있는지 여부를 조회할 때 사용된다.
	- 메인쿼리: 데이터테이블
	- 서브쿼리: 내역테이블
	- 메인쿼리에서 조회할 행이 서브쿼리의 테이블에 있는지(또는 없는지) 확인 => subquery의 where절
************************************************************************************* */
...
-- main query의 현제 행의 dept_id와 emp의 dept가 같은 것이 있는가 여부 => True/False
...
select dept_id,
       ...
       loc
from dept d
where exists (select emp_id from emp where dept_id = d.dept_id and salary >= 13000);
